...
function print_older_persons_letter() {
    ...
    writeAuditlog($_SESSION['userName'], $_SESSION['curr-id'], 'Older Persons Grant', 'Printing client letter for required Older Persons Grant docs.');

    // Send an SMS to the client too.
    $message = '*DEMO* Application for Older Persons Grant Started. Please obtain the required documentation and present to the SASSA. *DEMO*';
    $to = $_SESSION['cellNo'];
    sendSms($message, $to);
